...
test("issue-run issue-bound instructions include PR lifecycle requirements", () => {
  ...
  try {
    ...
    assert.match(
      instructions,
      /6\. PR lifecycle: open or update a PR for the issue branch, include "Fixes BRI-39" in the PR body for Linear linking, record the PR URL in run\.json changes\.pullRequestUrl, and verify the issue shows the PR link \(attach the PR URL to the issue manually if auto-linking is missing\)\./
    );
    assert.match(
      instructions,
